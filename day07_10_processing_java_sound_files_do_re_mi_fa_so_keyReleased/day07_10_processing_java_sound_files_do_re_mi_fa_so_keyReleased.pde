//day07_10_processing_java_sound_files_do_re_mi_fa_so_keyReleased
//修改自day07_09_processing_java_sound_files_do_re_mi_fa_so
...
} //要記得拉入do re mi fa so 的 wav 檔案進程式
int [] a = {0, 0, 0, 0, 0}; //a[i] 有沒有按下去?
void draw(){ 
  for (int i=0; i<5; i++){
    if (a[i]==1) fill(128);
    else fill(255); //有按下去變黑色，否則白色
    rect(i*100, 0, 100, 100);
  }
}
void keyReleased(){ //放開鍵盤按鍵
  if (key=='1') a[0]=0;
  if (key=='2') a[1]=0;
  if (key=='3') a[2]=0;
  if (key=='4') a[3]=0;
